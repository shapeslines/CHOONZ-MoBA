...
| 2 | **M5.1 `m5-command-replay`** — **in review, PR #69** (`slate-moba-phase5-m5.1.md`) — `Command`/`CommandReject`/`SimEvent`, ordering key, duplicate/stale/capacity rejection, live/replay parity | [plans/m5.1-command-replay.md](plans/m5.1-command-replay.md); fence `engine/sim/{include/sim/command.h,src/command.cpp}`, `tests/sim/command_replay_tests.cpp`; disjoint from rank 1 | open — parallel-capable |
| 3 | **`content-typed-payloads`** — validated hero/action/map/economy `.mba` payloads, byte-identical recook | [plans/content-typed-payloads.md](plans/content-typed-payloads.md); fence `engine/assets/`, `engine/asset_parsers/`, `tools/cooker/`, `tests/assets/`; ADR-0015 successor ADR required | **in review — PR #70** (ADR-0016; `slate-moba-phase5-content-payloads.md`) |
| 3b | **M5.2 `m5-hero-combat`** — `SimHeroDef` mirror, hero/projectile/status pools, `SimEvent` envelope, one `resolve_effect` pipeline, ATTACK/CAST commands | [plans/m5-hero-combat.md](plans/m5-hero-combat.md); fence `engine/sim/` hero/combat files; lint `sim_pipeline_owner` | **in review — PR #71** (stacked on #68/#69; `slate-moba-phase5-m5-hero-combat.md`; one logic-hash bump) |
| 4 | **ADR-0010 generated `asset_ids.gen.h` + build-time collision check** — cooker manifest mode; unpaid debt named in ADR-0016 | fence `tools/cooker/`, `cmake/`, `engine/asset_parsers/include/assets/`; ADR-0010 | open — small follow-on after the content lane merges |
| 4b | M4.2 PNG + own inflate (cooker-only) | ROADMAP M4.2; fence `tools/cooker/`, `engine/asset_parsers/` | open — pull forward only if M5.0 map authoring needs PNG |
| 3c | **M5.5 `m5-lane-objectives`** — `SimMatchDef`, team/minion/objective pools, waves, minion FSM, towers, death with killer, ledgers, match over | [plans/m5-lane-objectives.md](plans/m5-lane-objectives.md); fence `engine/sim/` objective files; `lane_slice.mapdesc` | **in review — PR #72** (stacked on #71; `slate-moba-phase5-m5-lane-objectives.md`; one logic-hash bump) |
| 5a | **`game-content-bridge`** — `.mba` hero/objective/economy records → `SimHeroDef`/`SimMatchDef` in `eng_game`; `content.h` static_assert bridge | plan to write from the merged `main`; fence `engine/game/`, `tests/game/` | open — next claim after #72 |
