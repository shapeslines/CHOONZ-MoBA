...
| ledger close | #26 successor | current candidate | this preserved report; current-main ledger and handoff remain authoritative |
...
- **Deferred with named owners (2):** G20 (cross-binary parity, M6.7) and G34
  (fuzz, M4.2/M6.1).
- **Closed after this sweep (2):** G11 and G13 were completed by the M3.4 structural
  determinism work now on main at `63f0b1d` (#38): the authoritative sim target owns
  the floating-point policy, and clang-cl/UBSan runs the same oracle.
...
- M3.4 is acceptance-complete on main at `63f0b1d` (#38), including the isolated
  authoritative sim target, floating-point/source/link boundary gates, shared
  10,000-tick self-check, and hosted clang-cl/UBSan capability proof.
- The current `docs/gap-close-ledger.md` and `docs/next-session.md` supersede this
  report for live sequencing. This document remains the PRE/PLAN/POST history of
  the 40-gap campaign; it does not authorize Phase 4 work or revive retired
  worktree/branch cleanup instructions.
