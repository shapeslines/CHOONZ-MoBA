...
static bool test_create_owned_directory(const char* label, OwnedTestDirectory* out) {
    ...
    for (uint32_t attempt = 0u; attempt < 64u; ++attempt) {
        ...
        const int chars = std::snprintf(
            candidate.path, sizeof(candidate.path), "moba_rooted_%s_%lu_%lu", label,
...
static bool test_release_owned_directory(OwnedTestDirectory* owned) {
