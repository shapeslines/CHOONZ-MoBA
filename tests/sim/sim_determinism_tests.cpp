...
static_assert(SIM_LOGIC_HASH == 0x5b47e648953a63fcULL,
...
TEST(sim_determinism, recorded_hash_stream_matches_independent_replay_for_10000_ticks) {
    ...
    CHECK(size == 134812);
    CHECK(recorded_final_hash == 0x36e6de56cb662dbaULL); // pins M3.2 canonical order + behavior
    if (size == 0) return;
...
TEST(sim_determinism, ten_k_tick_record_stays_inside_cpu_time_ceiling) {
    ...
    CHECK(size > 0);
    CHECK(final_hash == 0x36e6de56cb662dbaULL);   // oracle re-pinned in the smoke path
