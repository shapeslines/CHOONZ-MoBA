...
static_assert(SIM_LOGIC_HASH == 0xcef8548df2b2a518ULL,
...
TEST(sim_determinism, recorded_hash_stream_matches_independent_replay_for_10000_ticks) {
    ...
    CHECK(size == 134812);
    CHECK(recorded_final_hash == 0xff4e1ca0c779455bULL); // pins M3.2 canonical order + behavior
    if (size == 0) return;
...
TEST(sim_determinism, ten_k_tick_record_stays_inside_cpu_time_ceiling) {
    ...
    CHECK(size > 0);
    CHECK(final_hash == 0xff4e1ca0c779455bULL);   // oracle re-pinned in the smoke path
